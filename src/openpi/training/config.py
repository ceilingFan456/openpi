...
class DataConfig:
    ...
    # Optional local root directory for the LeRobot dataset. If set, data will be loaded
    # from this path instead of downloading from HuggingFace.
...
_CONFIGS = [
    ...
    # E1: Finetune on original Libero from blob storage (no HF download needed)
    TrainConfig(
        name="pi05_libero_blob",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
            assets=AssetsConfig(
    ...
    # Speed-varied Libero dataset for E2 experiment
    TrainConfig(
        name="pi05_libero_speed_varied",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
            assets=AssetsConfig(
...
###############################################################################################################################################################################################
    ...
    TrainConfig(
        ...
        name="pick_and_place_new_132_three_views_30k_steps",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=16,
    ...
    #
    # auxiliary 2D co-training (LeRobot format). equal weights for both losses. 
    #
    TrainConfig(
        name="pi05_aux2d_co_training_orange_cube_paired_25",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=16,
            enable_aux_2d=True,
    ...
    #
    TrainConfig(
        name="pi05_aux2d_co_training_baseline",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=16,
            enable_aux_2d=True,
